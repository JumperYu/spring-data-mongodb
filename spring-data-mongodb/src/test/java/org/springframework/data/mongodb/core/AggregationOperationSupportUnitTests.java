...
public class AggregationOperationSupportUnitTests {

	@Mock MongoTemplate template;
	AggregationOperationSupport opSupport;

	@Before
	public void setUp() {
		opSupport = new AggregationOperationSupport(template);
	}
	...
	public void throwsExceptionOnNullAggregation() {
		opSupport.aggregateAndReturn(Person.class).process(null);
	}

	@Test // DATAMONGO-1563
	public void aggregateWithUntypedAggregationAndExplicitCollection() {

		opSupport.aggregateAndReturn(Person.class).inCollection("star-wars").process(newAggregation(project("foo")));
	...
	public void aggregateWithUntypedAggregation() {
		...
		opSupport.aggregateAndReturn(Person.class).process(newAggregation(project("foo")));
	...
	public void aggregateWithTypeAggregation() {
		...
		opSupport.aggregateAndReturn(Jedi.class).process(newAggregation(Person.class, project("foo")));
	...
	public void aggregateStreamWithUntypedAggregationAndExplicitCollection() {
		...
		opSupport.aggregateAndReturn(Person.class).inCollection("star-wars").streamProcess(newAggregation(project("foo")));
	...
	public void aggregateStreamWithUntypedAggregation() {
		...
		opSupport.aggregateAndReturn(Person.class).streamProcess(newAggregation(project("foo")));
	...
	public void aggregateStreamWithTypeAggregation() {
		...
		opSupport.aggregateAndReturn(Jedi.class).streamProcess(newAggregation(Person.class, project("foo")));
